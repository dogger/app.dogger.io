...
{
    public class InstallSoftwareOnInstanceStage : SshInstanceStage, IInstallSoftwareOnInstanceStage
    {
        ...
        public InstallSoftwareOnInstanceStage(
        ...
        protected override async Task<ProvisioningStateUpdateResult> OnUpdateAsync(ISshClient sshClient)
        {
            if (this.IpAddress == null)
                throw new InvalidOperationException($"Must provide IP address to {nameof(InstallSoftwareOnInstanceStage)}");

            await InstallDockerAsync(sshClient);

            var instance = this.InstanceName != null ? 
                await this.mediator.Send(new GetInstanceByNameQuery(this.InstanceName)) :
                null;
